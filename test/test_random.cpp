...
void test_random::test_random_real()
{
    ...
    for (int i=0; i<100; ++i) {
        assert_in_range(randomA.value(), -0.01, 3.01, SPOT);
        assert_in_range(randomB.value(), 1.49, 4.21, SPOT);
    }
}
...
void test_random::test_random_container_list()
{
    ...
    unittest::random_choice<int, std::list<int>> random(a_list);
    const unsigned length = 5;
    const unsigned min_length = 3;
    unittest::random_container<int, std::list<int>> random_contA(random, length);
    unittest::random_container<int, std::list<int>> random_contB(random, min_length, length);
